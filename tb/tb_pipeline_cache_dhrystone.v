...
module tb_pipeline_cache_dhrystone;

    localparam integer MEM_WORDS = 65536;      // 256 KiB per memory
    localparam integer TIMEOUT_CYCLES = 3000000; // pipelined CPU needs more cycles than single-cycle
    ...
    initial begin
        $dumpfile("pipeline_cache_dhrystone.vcd");
        $dumpvars(0, tb_pipeline_cache_dhrystone);
        ...
        $display("Running Dhrystone benchmark with D-cache...");
        ...
        if (errors == 0) begin
            $display("PASS: RV32IM pipelined D-cache Dhrystone test passed.");
        end else begin
            $display("FAIL: RV32IM pipelined D-cache Dhrystone test had %0d error(s).", errors);
        end
