...
module tb_pipeline_axil_icache_dhrystone;
    ...
    wire [31:0] i_awaddr;  wire i_awvalid;  wire i_awready;
    ...
    initial begin
        $dumpfile("pipeline_axil_icache_dhrystone.vcd");
        $dumpvars(0, tb_pipeline_axil_icache_dhrystone);
        ...
        $display("Running Dhrystone benchmark with AXI-Lite I-cache + D-cache...");
        for (cycle = 0; cycle < TIMEOUT_CYCLES; cycle = cycle + 1) begin
            ...
            if (illegal_insn) begin
                ...
                $display("INFO: I-cache accesses=%0d hits=%0d misses=%0d", icache.access_count, icache.hit_count, icache.miss_count);
                $display("INFO: D-cache accesses=%0d hits=%0d misses=%0d writebacks=%0d", dcache.access_count, dcache.hit_count, dcache.miss_count, dcache.writeback_count);
        ...
        if (errors == 0) begin
            $display("PASS: RV32IM pipelined AXI-Lite I-cache + D-cache Dhrystone test passed.");
        end else begin
            $display("FAIL: RV32IM pipelined AXI-Lite I-cache + D-cache Dhrystone test had %0d error(s).", errors);
        end
        if (errors != 0)
            $fatal(1);
        $finish;
